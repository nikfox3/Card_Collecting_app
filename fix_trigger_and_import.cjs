...
console.log('🚀 Fixing database trigger and importing pricing data...');
...
db.run("DROP TRIGGER IF EXISTS cards_au", (err) => {
  if (err) {
    console.error('❌ Error dropping trigger:', err.message);
  } else {
    console.log('✅ Dropped old cards_au trigger');
  }
  
  // Step 2: Create a new trigger that handles NULL set_ids gracefully
  db.run(`
  ...
  `, (err) => {
    if (err) {
      console.error('❌ Error creating new trigger:', err.message);
      db.close();
      process.exit(1);
    }
    console.log('✅ Created new cards_au trigger with NULL handling');
    
    // Step 3: Now run the pricing import
    importPricingData();
  });
});
