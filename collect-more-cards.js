...
async function collectMoreCardPricing() {
  console.log('🚀 Starting additional card pricing collection...');
  console.log('💰 Credits available: ~16,477');
  
  const db = new sqlite3.Database(DB_PATH);
  const today = new Date().toISOString().split('T')[0];
  
  // Get more cards that don't have Pokemon Price Tracker data yet
  const cards = await getCardsToCollect(db, 2000); // Increase to 2000 cards
  ...
  try {
    for (let i = 0; i < cards.length; i++) {
      ...
      try {
        ...
        let delay = 300; // Increased base delay
        if (result.skipped) {
          delay = 150; // Shorter delay for skipped cards
        } else if (result.rateLimited) {
          delay = 2000; // Much longer delay for rate limits
        } else if (apiErrors > 5) {
          delay = 1000; // Longer delay if many errors
        } else if (apiErrors > 10) {
          delay = 2000; // Even longer delay if many errors
        }
        
        console.log(`  ⏳ Waiting ${delay}ms before next request...`);
        await new Promise(resolve => setTimeout(resolve, delay));
        
        // Progress update every 25 cards (more frequent)
        if ((i + 1) % 25 === 0) {
          console.log(`\n📈 Progress Update:`);
          ...
          console.log(`  💰 Estimated credits used: ~${successCount}`);
        }
...
async function getCardsToCollect(db, limit = 2000) {
  return new Promise((resolve, reject) => {
    ...
    db.all(`
      ...
      WHERE p.product_id IS NOT NULL
        ...
        AND p.ext_rarity != ''
        AND p.market_price > 5  -- Lowered threshold to $5 to get more cards
        ...
        AND (
          ...
          p.ext_rarity LIKE '%Rainbow%' OR
          p.ext_rarity LIKE '%Full Art%' OR
          p.ext_rarity LIKE '%Alternate%'
        )
  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 404) {
        ...
        console.log(`  ⚠️  Rate limited for product ${productId}`);
        return { productId, collectedCount: 0, rateLimited: true };
      }
...
// Run the collection
collectMoreCardPricing().catch(console.error);
