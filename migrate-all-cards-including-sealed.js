...
class CompleteCardsMigration {
  ...
  async migrate() {
    try {
      console.log('🔄 Starting Complete Cards Migration (Including Sealed Products)...\n');
      ...
      // Get ALL cards, not just those with abilities/attacks
      ...
      console.log(`📊 Found ${cards.length} total cards (including sealed products)\n`);
      ...
      for (const [index, card] of cards.entries()) {
        try {
          ...
          // Parse abilities and attacks from JSON (handle null/empty values)
          let abilities = [];
          let attacks = [];
          let images = {};
          ...
          try {
            ...
            // For sealed products, this is normal - they don't have abilities/attacks
            abilities = [];
            attacks = [];
            images = {};
          }
          ...
          if (index % 500 === 0) {
            console.log(`   Processed: ${card.name} (${card.set_name}) - ${card.supertype}`);
          }
      ...
      // Show breakdown by supertype
      console.log('\n📊 Breakdown by supertype:');
      const supertypeBreakdown = await all(`
        SELECT ext_card_type, COUNT(*) as count 
        FROM products 
        GROUP BY ext_card_type 
        ORDER BY count DESC
      `);
      
      for (const breakdown of supertypeBreakdown) {
        console.log(`   ${breakdown.ext_card_type}: ${breakdown.count} cards`);
      }
