...
// Script to update Pokemon cards that are missing attacks and abilities data
...
function generateAbilitiesForPokemon(cardName, types, stage, supertype) {
  ...
  // Add abilities based on stage - only for evolved Pokémon
...
// Update Pokemon cards that are missing attacks or abilities
async function updateMissingPokemonData() {
  try {
    console.log('🔄 Updating Pokemon cards that are missing attacks or abilities...\n');
    
    // Get Pokemon cards that are missing attacks OR abilities
    const missingDataCards = await runQuery(`
    ...
    console.log(`📋 Found ${missingDataCards.length} Pokemon cards missing attacks or abilities data`);
    
    let updated = 0;
    const batchSize = 100;
    
    // Process in batches
    for (let i = 0; i < missingDataCards.length; i += batchSize) {
      const batch = missingDataCards.slice(i, i + batchSize);
      console.log(`\n📦 Processing batch ${Math.floor(i/batchSize) + 1}/${Math.ceil(missingDataCards.length/batchSize)} (${batch.length} cards)...`);
    ...
    console.log(`  📋 Total processed: ${missingDataCards.length} cards`);
...
async function main() {
  try {
    console.log('🚀 Starting update for Pokemon cards missing attacks and abilities...\n');
    ...
    // Update missing Pokemon data
    await updateMissingPokemonData();
...
module.exports = { updateMissingPokemonData, generateAttacksForPokemon, generateAbilitiesForPokemon };
